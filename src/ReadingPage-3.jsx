...
export default function ReadingPage({ studentName, onSaveResult }) {
  ...
  const [passageChecked, setPassageChecked] = useState({}); // { passageIdx: true }
  const [demoRevealed, setDemoRevealed] = useState(false);
  const [exampleRevealed, setExampleRevealed] = useState(false);
  const [walkthroughRevealed, setWalkthroughRevealed] = useState(false);
  ...
  const openUnit = (id) => { setUnitId(id); setAnswers({}); setTextResults({}); setActiveTextIdx(0); setWalkthroughRevealed(false); };
        ...
        <div className="skillsWrap">
          <div className="skillsNav">
            {READING_SKILLS.map(sk => (
              <button key={sk.id} className={`skillsNavBtn${sk.id === activeSkillId ? " on" : ""}`} onClick={() => { setActiveSkillId(sk.id); setActivePassageIdx(0); setDemoRevealed(false); setExampleRevealed(false); setPracticeChecked(false); setPassageChecked({}); }}>
                {sk.title}
              </button>
            ))}
          </div>
            ...
            {s && (
              <div className="card" style={{ marginBottom: 16, padding: 20 }}>
                  ...
                  <div className="card" style={{ background: "rgba(76,175,136,.06)", border: "1px solid rgba(76,175,136,.25)", marginBottom: 16, padding: 16 }}>
                    ...
                    <div style={{ background: "#0b1622", borderRadius: 8, padding: 16, fontSize: 14, lineHeight: 1.9, marginBottom: 12 }}>
                      {s.demo.sentences.map((sent, i) => (
                        <span key={i} className={demoRevealed ? `demoSent ${sent.read ? "read" : "skip"}` : ""}>{sent.text}{" "}</span>
                      ))}
                    </div>
                    {!demoRevealed ? (
                      <button className="btn btn-sm" onClick={() => setDemoRevealed(true)}>Показать ответ 👁️</button>
                    ) : (
                      <>
                        <div style={{ fontSize: 12.5, color: "#c0b8a8", marginBottom: 8 }}>{s.demo.caption}</div>
                        <button className="btn btn-o btn-sm" onClick={() => setDemoRevealed(false)}>Скрыть</button>
                      </>
                    )}
                  </div>
                )}
                ...
                <div style={{ background: "rgba(197,155,68,.06)", border: "1px solid rgba(197,155,68,.2)", borderRadius: 8, padding: 14, marginBottom: (s.practice || s.passages) ? 16 : 0 }}>
                  ...
                  {!exampleRevealed ? (
                    <button className="btn btn-sm" onClick={() => setExampleRevealed(true)}>Показать ответ 👁️</button>
                  ) : (
                    <>
                      <div style={{ fontSize: 12.5, color: "#8a7d6d", marginBottom: 8 }}>{s.example.note}</div>
                      <button className="btn btn-o btn-sm" onClick={() => setExampleRevealed(false)}>Скрыть</button>
                    </>
                  )}
                </div>
                {s.practice && (
                  <div>
                    ...
                    {!practiceChecked ? (
                      ...
                      <div style={{ textAlign: "center", marginTop: 10 }}>
                        <button className="btn btn-o btn-sm" onClick={() => { setPracticeChecked(false); setSkillAnswers(a => { const c = { ...a }; s.practice.forEach(p => delete c[p.id]); return c; }); }}>Пройти ещё раз</button>
                      </div>
                    )}
                  </div>
                )}
                    ...
                    <div className="tabs" style={{ overflowX: "auto", flexWrap: "nowrap" }}>
                      {s.passages.map((psg, psgI) => (
                        <button key={psgI} className={`tab${psgI === activePassageIdx ? " on" : ""}`} style={{ whiteSpace: "nowrap" }} onClick={() => setActivePassageIdx(psgI)}>
                          {psg.title}{passageChecked[psgI] ? " ✓" : ""}
                        </button>
                      ))}
                    </div>
                    {(() => {
                      const psg = s.passages[activePassageIdx] || s.passages[0];
                      const done = !!passageChecked[activePassageIdx];
                      const allAnswered = psg.questions.every(p => skillAnswers[p.id] !== undefined);
                      return (
                          ...
                          {!done ? (
                            ...
                            <div style={{ textAlign: "center", marginTop: 10 }}>
                              <button className="btn btn-o btn-sm" onClick={() => {
                                setPassageChecked(c => { const cc = { ...c }; delete cc[activePassageIdx]; return cc; });
                                setSkillAnswers(a => { const cc = { ...a }; psg.questions.forEach(p => delete cc[p.id]); return cc; });
  ...
  return (
    <div>
      ...
      <div className="tabs" style={{ overflowX: "auto", flexWrap: "nowrap" }}>
        {unit.texts.map((t, ti) => (
          <button key={t.id} className={`tab${ti === activeTextIdx ? " on" : ""}`} style={{ whiteSpace: "nowrap" }} onClick={() => { setActiveTextIdx(ti); setWalkthroughRevealed(false); }}>
        ...
        <div key={ti} style={{ marginBottom: 26 }}>
          ...
          {task.walkthrough && (
            <div className="card" style={{ background: "rgba(76,175,136,.06)", border: "1px solid rgba(76,175,136,.25)", marginBottom: 16, padding: 16 }}>
              ...
              {!walkthroughRevealed ? (
                <button className="btn btn-sm" onClick={() => setWalkthroughRevealed(true)}>Показать ответ 👁️</button>
              ) : (
                <>
                  <div style={{ fontSize: 13.5, color: "#4caf88", fontWeight: 600, marginBottom: task.walkthrough.whyNotOthers ? 6 : 8 }}>
                    Ответ: {task.walkthrough.answer}
                  </div>
                  {task.walkthrough.whyNotOthers && (
                    <div style={{ fontSize: 12.5, color: "#8a7d6d", marginBottom: 8 }}>{task.walkthrough.whyNotOthers}</div>
                  )}
                  <button className="btn btn-o btn-sm" onClick={() => setWalkthroughRevealed(false)}>Скрыть</button>
                </>
              )}
            </div>
          )}
        ...
        <div className="sdiv">
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap", marginTop: 16 }}>
            ...
            {activeTextIdx < unit.texts.length - 1 && (
              <button className="btn" onClick={() => { setActiveTextIdx(activeTextIdx + 1); setWalkthroughRevealed(false); }}>Следующий текст →</button>
            )}
