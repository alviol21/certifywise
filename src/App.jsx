...
const TEACHER_PASSWORD = "certifywise2025"; // ← СМЕНИТЬ ПЕРЕД ДЕПЛОЕМ!
...
export default function App() {
  ...
  const [aiMsgs, setAiMsgs]       = useState([{ r: "a", t: "Привет! 👋 Я твой AI-репетитор по TKT, CELTA, IELTS и DELTA.\n\nЗадай любой вопрос: объясню термин, разберу ошибку, помогу подготовиться к конкретному модулю. Пиши на русском или английском!" }]);
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          system: `You are CertifyWise AI — an expert tutor specializing in TKT, CELTA, IELTS, and DELTA certification preparation for language teachers.

TKT: Modules 1-3 — language description, learner types, lesson planning, classroom management, assessment, error correction, feedback, resource types, teaching approaches, vocabulary/grammar/phonology teaching, CCQs, ICQs, drilling, eliciting, staging, scaffolding, differentiation.

CELTA: Language awareness (form/meaning/use), lesson planning (aims, staging, anticipated problems), teaching skills (rapport, monitoring, feedback, delayed error correction), classroom management, skills teaching (reading/listening/speaking/writing sub-skills), material adaptation.

IELTS: Academic Reading (T/F/NG, matching headings/information, gap-fill), Writing Task 1 (describing graphs, diagrams, maps — trends, overview), Writing Task 2 (argument types, task response, coherence, vocabulary), Listening (4 sections, question types, strategies), Speaking (3 parts, fluency, coherence, lexical resource, pronunciation, grammatical range).

DELTA: Module 1 (SLA theory: Krashen, Selinker, Vygotsky, Ellis, Lewis; methodology: CLT, TBL, PPP, lexical approach, audio-lingual; language analysis: phonology, morphology, syntax, discourse, pragmatics; assessment: validity, reliability, washback), Module 2 (professional practice, critical reflection, needs analysis, course design, experimental practice).

Respond in the same language the user writes in. Be precise and pedagogically accurate. Use concrete examples. Format answers clearly. Never refuse certification-related questions.`,
          messages: [
            ...aiMsgs.filter(m => m.r === "u" || m.r === "a").map(m => ({ role: m.r === "a" ? "assistant" : "user", content: m.t })),
            { role: "user", content: msg }
          ]
        })
      });
      const data = await res.json();
      const reply = data.content?.find(b => b.type === "text")?.text || "Извини, ответ не получен. Попробуй снова.";
      setAiMsgs(p => [...p, { r: "a", t: reply }]);
    } catch {
      setAiMsgs(p => [...p, { r: "a", t: "Ошибка соединения. Проверь интернет и попробуй снова." }]);
    }
    setAiLoad(false);
  };
  ...
  const PAGES = [["home","🏠 Главная"],["materials","📚 Материалы"],["practice","✏️ Тесты"],["listening","🎧 Listening"],["chat","🤖 AI-Репетитор"],["teacher","👩‍🏫 Учитель"]];
      ...
      <div className="wrap">
            ...
            {!name && (
              <div className="card" style={{marginBottom:22}}>
                ...
                <div className="row" style={{marginTop:13}}>
                  <input type="text" placeholder="Введи своё имя..." value={name} onChange={e => setName(e.target.value)} style={{flex:1}} />
        ...
        {page === "chat" && (
          <div>
            ...
            <p className="sb">Задай любой вопрос по TKT, CELTA, IELTS или DELTA</p>
            <div style={{display:"flex",gap:7,marginBottom:14,flexWrap:"wrap"}}>
              {["Объясни second conditional для CELTA","Стратегии IELTS Writing Task 2","Что такое fossilization в DELTA?","TKT: что такое ICQ и CCQ?","Как исправлять ошибки студентов?"].map(q => (
            ...
            <div className="chbox" ref={chatRef}>
              ...
              {aiLoad && <div className="msg a"><div className="bub" style={{color:"#8a7d6d"}}>⌛ Думаю...</div></div>}
            </div>
            <div className="chin">
              <input className="chinput" type="text" placeholder="Задай вопрос по TKT, CELTA, IELTS или DELTA..." value={aiIn} onChange={e => setAiIn(e.target.value)} onKeyDown={e => e.key === "Enter" && sendAi()} />
          ...
          <div>
            ...
            {!tMode ? (
              <div className="card">
                ...
                <p style={{fontSize:12,color:"#4a5560",marginTop:10}}>Пароль по умолчанию: <code style={{color:"#c59b44"}}>certifywise2025</code> — смени в коде перед деплоем (константа TEACHER_PASSWORD)</p>
              </div>
            ) : (
              <div>
...
/* ═══════════════════════════════════════════════════════════════
   ...
   СТРУКТУРА ФАЙЛОВ (создай на компьютере):
   ...
   │   └── main.jsx    ← см. ниже
   ├── index.html      ← см. ниже
   ├── package.json    ← см. ниже
   └── vite.config.js  ← см. ниже
   ...
   ── ДЕПЛОЙ ──
   1. Загрузи все файлы на github.com (новый репозиторий "certifywise")
   2. Зайди на vercel.com → Add New Project → импортируй из GitHub
   3. Framework: Vite → Deploy
   4. Получишь ссылку certifywise.vercel.app — поделись со студентами!

   ВАЖНО: Смени TEACHER_PASSWORD перед деплоем!
═══════════════════════════════════════════════════════════════ */
