...
export default function App() {
  ...
  const [teacherTab, setTeacherTab] = useState("progress");
  const [allChats, setAllChats]   = useState([]); // все сообщения всех студентов (для учителя)
  const [chatProfiles, setChatProfiles] = useState({}); // { user_id: { full_name, chat_cleared_at } }
  ...
  useEffect(() => {
    ...
    supabase.from("chat_messages").select("*").eq("user_id", session.user.id).order("created_at", { ascending: true }).limit(200)
      .then(({ data, error }) => {
        if (error || !data) return;
        const cutoff = profile.chat_cleared_at ? new Date(profile.chat_cleared_at) : null;
        const visible = cutoff ? data.filter(m => new Date(m.created_at) > cutoff) : data;
        if (visible.length === 0) return; // нет истории — остаётся приветственное сообщение
        setAiMsgs(visible.map(m => ({ r: m.role, t: m.text })));
      });
  }, [session, profile]);

  const saveChatMsg = async (role, text) => {
    if (!session) return;
    await supabase.from("chat_messages").insert({ user_id: session.user.id, role, text });
  };

  const clearChatHistory = async () => {
    if (!session) return;
    const now = new Date().toISOString();
    await supabase.from("profiles").update({ chat_cleared_at: now }).eq("id", session.user.id);
    setProfile(p => ({ ...p, chat_cleared_at: now })); // данные в базе остаются — скрывается только у студента в интерфейсе
    setAiMsgs([{ r: "a", t: "Привет! 👋 Я твой персональный AI-тьютор по TKT, CELTA, IELTS и DELTA.\n\nЯ знаю твои результаты тестов и подстроюсь под тебя. Напиши «анализ» — и я разберу твои слабые места и составлю план подготовки. Или просто задай любой вопрос — пиши как тебе удобно!" }]);
  };
...
DELTA: SLA (Krashen, Selinker, Vygotsky, Ellis, Lewis), methodology (CLT, TBL, PPP, Lexical Approach), language analysis, Module 2 professional practice`;
  ...
  const sendAi = async () => {
    ...
    try {
      ...
      const errText = `Ошибка: ${e.message || "проверь API ключ и интернет"}`;
      setAiMsgs(p => [...p, { r: "a", t: errText }]);
      saveChatMsg("a", errText);
    }
    setAiLoad(false);
  };
        ...
        {page === "chat" && (
          <div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              ...
              {aiMsgs.length > 1 && (
                <button className="btn btn-o btn-sm" onClick={() => { if (window.confirm("Удалить всю историю переписки?")) clearChatHistory(); }}>🗑️ Очистить историю</button>
              )}
            </div>
          ...
          <div>
            ...
            {profile?.role !== "teacher" ? (
              ...
              <div>
                ...
                <div className="tabs">
                  <button className={`tab${teacherTab==="progress"?" on":""}`} onClick={() => setTeacherTab("progress")}>📊 Прогресс</button>
                  <button className={`tab${teacherTab==="chats"?" on":""}`} onClick={() => setTeacherTab("chats")}>💬 Чаты с AI</button>
                </div>
                {teacherTab === "chats" ? (
                  <div>
                    {allChats.length === 0 ? (
                      <div className="empty">Пока никто не писал AI-тьютору.</div>
                    ) : selectedChatStudent ? (
                      <div>
                        <button className="btn btn-o btn-sm" style={{marginBottom:14}} onClick={() => setSelectedChatStudent(null)}>← Все студенты</button>
                        <h3 style={{marginBottom:10}}>{chatProfiles[selectedChatStudent]?.full_name || "Студент"}</h3>
                        <div style={{display:"flex",flexDirection:"column",gap:10}}>
                          {allChats.filter(m => m.user_id === selectedChatStudent).map((m, i, arr) => {
                            const cleared = chatProfiles[selectedChatStudent]?.chat_cleared_at;
                            const showMarker = cleared && i > 0 && new Date(arr[i-1].created_at) <= new Date(cleared) && new Date(m.created_at) > new Date(cleared);
                            return (
                              <div key={m.id}>
                                {showMarker && (
                                  <div style={{textAlign:"center",fontSize:11.5,color:"#8a7d6d",margin:"10px 0",borderTop:"1px dashed rgba(197,155,68,.3)",paddingTop:8}}>
                                    — студент скрыл историю у себя ({new Date(cleared).toLocaleString("ru-RU")}) — у вас видно всё —
                                  </div>
                                )}
                                <div style={{background: m.role==="u" ? "rgba(197,155,68,.08)" : "#12253b", border:"1px solid rgba(255,255,255,.06)", borderRadius:10, padding:"10px 14px"}}>
                                  <div style={{fontSize:11,color:"#8a7d6d",marginBottom:4}}>{m.role==="u"?"Студент":"AI"} · {new Date(m.created_at).toLocaleString("ru-RU")}</div>
                                  <div style={{fontSize:13.5,color:"#e8dfd0",whiteSpace:"pre-wrap"}}>{m.text}</div>
                                </div>
                              </div>
                            );
                          })}
                      ...
                      <div style={{overflowX:"auto"}}>
                        <table>
                          <thead><tr><th>Студент</th><th>Сообщений</th><th>Последняя активность</th><th></th></tr></thead>
                          <tbody>
                            {[...new Set(allChats.map(m => m.user_id))].map(uid => {
                              const msgs = allChats.filter(m => m.user_id === uid);
                              const last = msgs[msgs.length - 1];
                              return (
                                <tr key={uid}>
                                  <td>{chatProfiles[uid]?.full_name || "Студент"}</td>
                                  <td>{msgs.length}</td>
                                  <td style={{fontSize:12,color:"#4a5560"}}>{new Date(last.created_at).toLocaleString("ru-RU")}</td>
                                  <td><button className="btn btn-o btn-sm" onClick={() => setSelectedChatStudent(uid)}>Открыть</button></td>
                                </tr>
                              );
                            })}
                          </tbody>
